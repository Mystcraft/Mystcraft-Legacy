...
public class ComponentVillageArchivistHouse extends StructureVillagePieces.Village {
    ...
    private void placeDeskAt(World worldObj, int i, int j, int k, int i2, int k2, StructureBoundingBox boundingbox) {
        ...
        this.setBlockState(worldObj, block.getDefaultState().withProperty(BlockWritingDesk.ROTATION, facing),
                i, j, k, boundingbox);
        this.setBlockState(worldObj, block.getDefaultState().withProperty(BlockWritingDesk.ROTATION, facing).withProperty(BlockWritingDesk.IS_FOOT, true),
                i2, j, k2, boundingbox);
    }
