...
public class IOInventory implements IItemHandlerModifiable {
	...
	private final TileEntityBase owner;
	...
	private Map<Integer, InventoryFilter> filteredSlots = new HashMap<>();
	private int[] inSlots = new int[0], outSlots = new int[0],
			miscSlots = new int[0];
	...
	public IOInventory(TileEntityBase owner, int[] inSlots, int[] outSlots) {
		this(owner, inSlots, outSlots, EnumFacing.VALUES);
	}

	public IOInventory(TileEntityBase owner, int[] inSlots, int[] outSlots, EnumFacing... accessibleFrom) {
		this.owner = owner;
	...
	public IOInventory applyFilter(InventoryFilter filter, int... slots) {
		for (int slot : slots) {
			this.filteredSlots.put(slot, filter);
		}
		return this;
	}

	public TileEntityBase getOwner() {
		return owner;
	}
	...
	public ItemStack insertItem(int slot, @Nonnull ItemStack stack, boolean simulate) {
		...
		InventoryFilter filter = this.filteredSlots.get(slot);
		if (filter != null) {
			if (!filter.canAcceptItem(slot, stack)) {
				return stack;
			}
		}
	...
	public static IOInventory deserialize(TileEntityBase owner, NBTTagCompound tag) {
