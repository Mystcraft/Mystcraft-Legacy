...
public class TileEntityDesk extends TileEntityBase implements InventoryFilter, ITickable {
	...
	protected IOInventory buildWorkInventory() {
		return new IOInventory(this, new int[] { slot_pap }, new int[] {}, EnumFacing.VALUES).setMiscSlots(slot_wrt, slot_ctn, slot_out).setListener(() -> onChange(true)).applyFilter(this, slot_pap); //Doesn't matter for any other slots anyway
	}

	protected IOInventory buildTabInventory() {
		IOInventory inv = new IOInventory(this, new int[0], new int[0], EnumFacing.VALUES).setListener(() -> onChange(false));
		for (int i = 0; i < 25; i++) {
			inv.setMiscSlots(i);
		}
		return inv;
	...
	public ItemStack getTabItem(byte activeslot) {
		if (activeslot < 0 || activeslot >= inventoryTabItems.getSlots())
		...
		if (itemstack.getItem() instanceof IItemPageCollection)
			return itemstack;
		if (itemstack.getItem() instanceof IItemWritable)
			return itemstack;
		return ItemStack.EMPTY;
	}
	...
	public boolean canAcceptItem(int slot, @Nonnull ItemStack stack) {
		...
		if (slot < 0)
			return false;
		if (slot >= inventoryTabItems.getSlots())
			return false;
		if (stack.getItem() instanceof IItemPageCollection)
			return true;
		if (stack.getItem() instanceof IItemWritable)
			return true;
		return false;
	}

	public boolean hasTop() {
		if (!world.getBlockState(pos.up()).getBlock().equals(ModBlocks.writingdesk))
			return false;
		if (!BlockWritingDesk.isBlockTop(world.getBlockState(pos.up())))
			return false;
		return true;
	}
	...
	public boolean isLeftCovered() {
		...
		if (!world.isAirBlock(pos.up()) && !state.getBlock().equals(ModBlocks.writingdesk))
			return true;
	...
	public List<ItemStack> getBookPageList(EntityPlayer player) {
		...
		if (itemstack.isEmpty())
			return null;
		if (!(itemstack.getItem() instanceof IItemPageProvider))
			return null;
	...
	public ItemStack addPageToTab(EntityPlayer player, @Nonnull ItemStack itemstack, @Nonnull ItemStack page) {
		...
		if (itemstack.getItem() instanceof IItemPageAcceptor)
			result = ((IItemPageAcceptor) itemstack.getItem()).addPage(player, itemstack, page);
		if (result == page)
			return result;
		this.markForUpdate();
		return result;
		...
		if (!this.inventoryStacks.getStackInSlot(slot_ctn).isEmpty()) {
			...
			if (far.isSuccess()) {
				...
				if (mergeItemStacksLeft(this.inventoryStacks.getStackInSlot(slot_out), far.getResult()) != this.inventoryStacks.getStackInSlot(slot_out)) {
					this.inventoryStacks.setStackInSlot(slot_out, mergeItemStacksLeft(this.inventoryStacks.getStackInSlot(slot_out), FluidUtil.tryFillContainer(container, inkwell, Fluid.BUCKET_VOLUME, null, true).getResult()));
