...
public class DebugUtils {
	...
	public static IDebugElement getElement(String address) {
		...
		for (; i < array.length - 1; ++i) {
			...
			if (!(next instanceof DebugNode)) throw new RuntimeException(String.format("Cannot expand debug element to address %s as a non-node type lies in the address (@%s).", address, array[i]));
			current = (DebugHierarchy.DebugNode) next;
		}
		if (array.length <= i) return current;
	...
	public static void register(String address, IDebugElement elem) {
		...
		for (; i < array.length - 1; ++i) {
			...
			if (!(next instanceof DebugNode)) throw new RuntimeException(String.format("Cannot add debug element to address %s as a non-node type lies in the address (@%s).", address, array[i]));
			current = (DebugHierarchy.DebugNode) next;
		}
